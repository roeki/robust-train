...
def main(_):
    ...
    lr_scheduler1 = (train_sum / batchsize) * lr_scheduler1
    lr_scheduler2 = (train_sum / batchsize) * lr_scheduler2
    ...
    while itr < itr_max:  # itr数除以250 即周期数
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if itr > (train_sum / batchsize) * schedule_start:
                ...
                # 8/255扰动时训练参数,batchsize=500时的数据 如果为200则均乘2/5
                # if (predicted_ori == labels).sum().item() >= 190:
                #     loss += k * criterion(outputs, labels)
                # else:
                #     if (predicted_ori == predicted_veri).sum().item() >= 375:
                #         loss = (criterion(outputs, labels)+criterion(outputs_original, labels))/2
                #     else:
                #         loss = criterion(outputs, predicted_ori)
                if (predicted_ori == labels).sum().item() >= 160:
                    loss += k * criterion(outputs, labels)
                else:
                    if (predicted_ori == predicted_veri).sum().item() >= 375:
                        loss += k * (criterion(outputs, labels) + criterion(outputs_original, labels)) / 2
                    else:
                        loss = criterion(outputs, predicted_ori)
            loss.backward()
            optimizer.step()
        ...
        if print_accuracy(net, trainloader, testloader, device, test=True, eps=eps_test) > best_verified_acc_flag:
            ...
            torch.save(net.state_dict(), 'models/cifar_small_convnet_16eps_ours8.pth')
